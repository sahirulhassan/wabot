...
async function sendMessages(numbersPath, mediaPath, message) {
    if (!numbersPath || !message) {
        console.error('Please provide both numbers and media paths.');
        return;
    }
...
async function sendToNumber(number, payload) {
    ...
    if (!id) console.warn(`SKIPPING: Number ${number} is not a WhatsApp user.`);

    if (payload.media) {
        return client.sendMessage(id._serialized, payload.media, { caption: payload.text });
    } else {
        return client.sendMessage(id._serialized, payload.text);
    }
}
